    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl p-5 mb-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <input className="border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="Nombre" value={name} onChange={(e) => setName(e.target.value)} required />
              <select className="border border-gray-300 rounded-lg px-3 py-2 text-sm" value={parentId} onChange={(e) => setParentId(e.target.value)}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <input type="number" className="border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="Posición" value={position} onChange={(e) => setPosition(Number(e.target.value))} />
          ...
          <div>
            <p className="text-sm text-gray-600 mb-2">Disponible en:</p>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        {rootCats.length === 0 ? (
          ...
          return (
            <div key={cat.id} className={`border-b border-gray-100 last:border-0 ${isEditing ? "bg-blue-50" : "hover:bg-gray-50"}`}>
              ...
              {expanded.has(cat.id) && subs.length > 0 && (
                <div className="ml-6 border-l-2 border-gray-100">
                  {subs.map((sub: any) => (
                    <div key={sub.id} className={`flex items-center gap-2 px-4 py-2.5 border-b border-gray-50 last:border-0 ${editing?.id === sub.id ? "bg-blue-50" : "hover:bg-gray-50"}`}>
                      <div className="w-4" />
