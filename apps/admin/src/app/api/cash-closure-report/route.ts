...
function buildFinancialReportMessage(report: any) {
  if (!report || report.error) return "";
  const owner = report.owner_profit || {};
  const sales = report.ventas || {};
  const summary = report.resumen_ejecutivo || {};
  const kpis = report.financial_kpis || {};
  const insights = Array.isArray(report.financial_insights)
    ? report.financial_insights.slice(0, 4)
    : [];

  return [
    "*Reporte financiero diario*",
    report.rango_operativo?.etiqueta ? `Rango: ${report.rango_operativo.etiqueta}` : null,
    "",
    `Pedidos: *${sales.total_pedidos || 0}*`,
    `Venta neta sin envio: *${money(Number(summary.ingresos_netos || sales.total || 0))}*`,
    `Envios: ${money(Number(sales.envio || 0))}`,
    `CMV: ${money(Number(summary.cmv || 0))}`,
    `Ganancia bruta: ${money(Number(summary.ganancia_bruta || owner.gross_profit || 0))}`,
    "",
    `Labor: ${money(Number(owner.labor_cost || 0))}`,
    `Packaging: ${money(Number(owner.packaging_cost || 0))}`,
    `Costos fijos: ${money(Number(owner.fixed_cost_allocated || 0))}`,
    `Ganancia operativa: *${money(Number(owner.operating_profit || 0))}*`,
    `Ganancia por pedido: ${money(Number(owner.profit_per_order || 0))}`,
    `Margen operativo: ${Number(kpis.operating_margin_pct || owner.operating_margin || 0).toFixed(1)}%`,
    `Margen contribucion: ${Number(kpis.contribution_margin_pct || owner.contribution_margin_pct || 0).toFixed(1)}%`,
...
async function fetchFinancialReport(req: NextRequest, tenantId: string, date: string, branchUuid: string | null) {
  const reportUrl = new URL("/api/reports/daily", req.url);
  reportUrl.searchParams.set("tenantId", tenantId);
  reportUrl.searchParams.set("date", date);
  if (branchUuid) reportUrl.searchParams.set("branchId", branchUuid);
...
export async function POST(req: NextRequest) {
  ...
  if (closureId) {
    ...
    const financialReport = await fetchFinancialReport(req, user.tenant_id, closureBusinessDate, hydratedClosure.branch_id);
    const financialMessage = buildFinancialReportMessage(financialReport);
    ...
    const financialReport = await fetchFinancialReport(req, user.tenant_id, date, requestedBranchId);
    const financialMessage = buildFinancialReportMessage(financialReport);
    message = [
